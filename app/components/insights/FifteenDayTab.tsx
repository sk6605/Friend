...
export default function FifteenDayTab({ userId, totalDays }: { userId: string; totalDays: number }) {
  const today = new Date().toISOString().slice(0, 10);
  const fifteenAgo = new Date(Date.now() - 15 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);

  const [startDate, setStartDate] = useState(fifteenAgo);
  ...
  const [fetchStart, setFetchStart] = useState(fifteenAgo);
  const [fetchEnd, setFetchEnd] = useState(today);

  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/insights?userId=${userId}&tab=15day&startDate=${fetchStart}&endDate=${fetchEnd}`);
      ...
      if (!json.available) {
        ...
        setAvailable(true);
        setEarliestDate(json.earliestDate);
        setData(json.data);
      }
    } catch { /* ignore */ }
    setLoading(false);
  ...
  if (!available) {
    return <TabNotAvailable tabName="15-Day" daysNeeded={15} daysHave={totalDays} />;
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3">
        <h3 className="text-sm font-semibold text-neutral-700 dark:text-neutral-300">15-Day Analysis</h3>
        <DateRangePicker
          startDate={startDate}
          endDate={endDate}
          ...
          onApply={() => { setFetchStart(startDate); setFetchEnd(endDate); }}
        />
      </div>
