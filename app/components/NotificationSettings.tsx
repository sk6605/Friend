...
export default function NotificationSettings({ userId }: { userId: string }) {
  ...
  // Fetch user plan + push subscription state from DB
  useEffect(() => {
    const fetchState = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Read push subscription state from DB — source of truth
  ...
  const handleToggle = useCallback(async () => {
    ...
    try {
      if (isSubscribed) {
        // === UNSUBSCRIBE ===
        // 1. Update DB first (immediate effect on cron notifications)
        ...
        // 2. Opt out of OneSignal in background (best effort)
        ...
        // === SUBSCRIBE ===
        // 1. Try to get OneSignal and request permission
        const os = getOneSignal();
        if (os) {
          try {
            ...
            // OneSignal might not be available, proceed with DB update anyway
          }
        }

        // 2. Update DB — this ensures cron jobs will send push via OneSignal API
