      ...
      ">
          ...
          <div className="flex items-center justify-between mb-4 px-1">
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col flex-1">
                <span className="text-sm font-semibold text-neutral-800 dark:text-white/90 leading-tight truncate">{aiName || 'Lumi'}</span>
                <div className="flex items-center gap-1.5 mt-0.5 max-w-[120px]">
                  {nickname && <span className="text-[11px] text-neutral-500 dark:text-slate-500 leading-tight truncate">{nickname}</span>}
                  <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-bold uppercase shrink-0 
                    ${planName === 'premium' ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400 border border-amber-200 dark:border-amber-500/30' 
                    : planName === 'pro' ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/30' 
                    : 'bg-neutral-100 text-neutral-500 dark:bg-white/10 dark:text-neutral-400'}
                  `}>
                    {planName === 'premium' ? '👑 PREM' : planName === 'pro' ? '⚡ PRO' : 'FREE'}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="border-t border-purple-100/30 dark:border-purple-800/15 p-3 space-y-2 bg-white/30 dark:bg-transparent safe-bottom">
          ...
          >
            ...
            {planName === 'free' ? 'Upgrade Plan' : `Manage My ${planDisplayName}`}
          </Link>
          <Link
            href="/insights"
            className="
