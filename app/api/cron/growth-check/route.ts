...
/**
 * GET /api/cron/growth-check
 * Cron Job: Runs weekly to analyze emotional trends and generate interventions.
 *
 * Logic:
 * 1. Analyzes last 7 days of Daily Insights.
 * 2. Calculates mood trends, trigger frequencies, and thinking patterns.
 * 3. If negative trends detected, uses OpenAI to generate actionable advice (CBT/Behavioral).
 * 4. Creates a `GrowthReport` and sends a notification.
 *
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Find users with enough data (>= 3 daily insights in past 7 days) and dataControl enabled
    ...
    for (const user of users) {
      ...
      // Only generate intervention if concerning signals detected
      ...
      // Generate targeted intervention via GPT
...
Recent moods: ${insights.map(i => `${i.date.toISOString().slice(0, 10)}: ${i.mood || 'unknown'} (${i.moodScore || '?'}/10)`).join(', ')}`,
      ...
      // Create growth_nudge notification
